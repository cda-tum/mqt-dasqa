...
class Capacitor:
    ...
    def generate_capacitor(self):
        ...
        for x in range(N_x):
            for y in range(N_y):
                ...
                capacitor_name = f"Cap_Readout_{current_qubit}"
                cap_readout = CapNInterdigital(
                    self._design,
                    capacitor_name,
                    options=dict(pos_x=pos_x, pos_y=pos_y, orientation=orientation),
                )
                capacitors[capacitor_name] = cap_readout
    ...
    def _get_configuration(self, N_y, x, y):
        ...
        if y == 0:
            ...
            offset_x = 300
            pos_x = str(x * x_loc - x_loc / 2 + offset_x) + "um"
