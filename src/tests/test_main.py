...
class TestMain:
    ...
    @patch("src.concrete_design_flow1.ConcreteDesignFlow1")
    def test_main(self, mock_class1):
        from src.__main__ import flow

        args = argparse.Namespace(
            file_path="./src/tests/test_circuit/circuit1.qasm",
            config_file_path="./src/tests/test_config/config.yml",
        )
        flow(args)

    # Check for no exceptions
    @patch("src.architecture_generator1.generator.Generator")
    ...
    def test_main_valid_circuit_file_path(self, capfd):
        from src.__main__ import check_file_path

        args = argparse.Namespace(file_path="./src/tests/test_circuit/circuit1.qasm")
    ...
    def test_main_valid_config_file_path(self, capfd):
        ...
        args = argparse.Namespace(file_path="./src/tests/test_config/config.yml")
        config_absolute_path, exit_code = check_file_path(args)
        assert exit_code == 0

    def test_main_invalid_config_file_path(self, capfd):
        from src.__main__ import check_config_file_path
    ...
    def test_load_yaml(self):
        from src.__main__ import load_yaml
